function passiveGratingAnalysisFunc(Animal, expID, expID_ret, sessNr, expInfoNr, hva)
...
if strcmp(expID,'02-Mar-2023_1')
    load (['Y:\haider\Data\analyzedData\EKK\WFI\' Animal filesep 'gratings' filesep expID filesep 'old' filesep num2str(sessNr) filesep 'preprocessed_WFIdata_' num2str(sessNr, '%04i') '.mat'])
else
    load(['Y:\haider\Data\analyzedData\EKK\WFI\' cPath filesep 'preprocessed_WFIdata_' num2str(sessNr, '%04i') '.mat'])
end

expPath = ['Y:\haider\Data\Behavior\expInfo\' Animal filesep date filesep num2str(expInfoNr)];        
sessID = [date '_' num2str(expInfoNr) '_' Animal];
...
multiContrast = true; % true for multi contrast analysis 
...
[avgStimResponse, avgTempTrace, binocOnFrame, monocOnFrame] = barmapMultiCont_gratings(paramSorted, allData, bFrameTimes, savePath, 6, img,sessNr, expID_ret, flag, hva); % you can also apply full contrast grating
end