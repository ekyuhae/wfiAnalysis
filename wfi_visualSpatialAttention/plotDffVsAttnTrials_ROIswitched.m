...
for cDay = 1: length(expID)
    ...
    for s = 1: length(cSess)
        ...
        if idx == [1 1] % the session had attn signs for both blocks
            ...
            for i = 1: length(binocTemp)
                indivTempTrace(s).binoc{i} = getIndividTrace(binocTemp{i},btraceLength, allData, roi.monoc, cord, sPath, flag); % roi swapped
                avgTempTrace(s).binoc(i,:) = mean(indivTempTrace(s).binoc{i},1);
            end
            for i = 1: length(monocTemp)
                indivTempTrace(s).monoc{i} = getIndividTrace(monocTemp{i},mtraceLength, allData, roi.binoc, cord, sPath, flag); % roi swapped
    ...
    if exist('mTrNr', 'var') && exist('bTrNr', 'var')
        save(['hit_aligned_dff_ROIswitched.mat'],'avgTempTrace', 'bTrNr', 'mTrNr', 'bSess', 'mSess');
    elseif exist('mTrNr', 'var') && ~exist('bTrNr', 'var')
        save(['hit_aligned_dff_ROIswitched.mat'],'avgTempTrace', 'mTrNr', 'bSess', 'mSess');
    elseif ~exist('mTrNr', 'var') && exist('bTrNr', 'var')
        save(['hit_aligned_dff_ROIswitched.mat'],'avgTempTrace', 'bTrNr', 'bSess', 'mSess');
    end
    ...
    if exist("avgSessionB", "var")
        ...
        sgtitle(['Binoc Attentional block with Monoc ROI: sess ' strsplit(num2str(sessNr{cDay,1}), ' ')])

        savefig(gcf, 'dffAcrossBinocHitTrials_ROIswitched')
    ...
    if exist("avgSessionM", "var")
        ...
        sgtitle(['Monoc Attentional block with Binoc ROI: sess ' strsplit(num2str(sessNr{cDay,2}), ' ')])
        savefig(gcf, 'dffAcrossMonocHitTrials_ROIswitched')
    ...
    if exist("bTrNr", "var")
        ...
        savefig(gcf, 'peak_vs_BinocHitTrialNumber_ROIswitched')
        % saveas(gcf, 'peak_vs_BinocHitTrialNumber_ROIswitched.png')
    end
    ...
    if exist("mTrNr", "var")
        ...
        set(findobj(gcf, 'type', 'axes'), 'FontSize', 12)
        savefig(gcf, 'peak_vs_MonocHitTrialNumber_ROIswitched')
