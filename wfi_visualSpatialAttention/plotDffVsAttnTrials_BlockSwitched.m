...
Animal = 'M230831_1';
expID = {'17-Aug-2023' '21-Aug-2023' '23-Aug-2023' '30-Aug-2023' '01-Sep-2023' '04-Sep-2023' '26-Sep-2023'...
...
a.binoc ={[1]; [1 3]; [4]; [5]; []; [3]; [6 7]; []; [5 6]; [2 3 4]; [1 2 4]; []};
a.monoc = {[]; []; []; [2]; [2]; []; []; [9 10 11]; []; []; [1 2 3]; [5 6 7]} ; % WFI session Number 

b.binoc = {[1]; [1 3]; [4]; [5]; []; [3]; [6 7]; []; [5 6]; [2 3 4]; [1 2 4]; []};
b.monoc = {[]; []; []; [2]; [2]; []; []; [9 10 11]; []; []; [1 2 3]; [5 6 7]} ; % expInfo session Number
...
if ~isfolder('attention_over_days')
    if isempty(hva)
        ...
        mkdir(['attention_over_days_' hva]) % this line does not matter since it's only to load the meta data
    end 
end
...
%% allocate sessions that had attentional signs in either monoc/binoc to the opposite
% binoc ROI's response to monoc blocks that had binoc attention & vice versa
% original sessNr = {binoc attn, monoc attn} 
% -> switch to {monoc attn, binoc attn} and use the original ROI [binoc.roi, monoc.roi]) 
% sessNr_oppBlock(:,1) = sessNr(:,2);
% sessNr_oppBlock(:,2) = sessNr(:,1);
% clear sessNr 
% sessNr = sessNr_oppBlock;
%% pulling the analyzed mat file (output from running wfiBehavior) 
for cDay = 1:length(expID)
    ...
    for s = 1: length(cSess)
        ...
        if idx == [1 0] % binoc attn session
            ...
            sPath= pwd;
            for i = 1: length(monocTemp)
                indivTempTrace(s).monoc{i} = getIndividTrace(monocTemp{i},mtraceLength, allData, roi.binoc, cord, sPath, flag);
                avgTempTrace(s).monoc(i,:) = mean(indivTempTrace(s).monoc{i},1);
            end
            monocTemp= []; mHitTimes =[];
        elseif idx == [0 1] % monoc

            [bHitTimes, ~] = behSortParameter_attn(trTable, block, bFrameTimes, vFrameTimes, stimOn); % looking at binoc block 
            bTrNr(s) = max(trTable.trNumInBlock (trTable.boolBinoc));
            ...
            sPath= pwd;
            for i = 1: length(binocTemp)
                indivTempTrace(s).binoc{i} = getIndividTrace(binocTemp{i},btraceLength, allData, roi.monoc, cord, sPath, flag);
                avgTempTrace(s).binoc(i,:) = mean(indivTempTrace(s).binoc{i},1);
            end
            binocTemp =[]; bHitTimes =[]; 
        end
    ...
    if isfield(avgTempTrace, 'binoc') % monoc ROI response to binoc non attional block
    ...
    if isfield(avgTempTrace, 'monoc') % binoc ROI response to monoc non attnional block 
    ...
    if exist('mTrNr', 'var') && exist('bTrNr', 'var')
        save(['hit_aligned_dff_blockSwitched.mat'],'avgTempTrace', 'bTrNr', 'mTrNr', 'bSess', 'mSess');
    elseif exist('mTrNr', 'var') && ~exist('bTrNr', 'var')
        save(['hit_aligned_dff_blockSwitched.mat'],'avgTempTrace', 'mTrNr', 'bSess', 'mSess');
    elseif ~exist('mTrNr', 'var') && exist('bTrNr', 'var') 
        save(['hit_aligned_dff_blockSwitched.mat'],'avgTempTrace', 'bTrNr', 'bSess', 'mSess');
    end
    ...
    if exist("avgSessionB", "var") % monoc ROI response to binoc block (non attn)
        figure ;
        ...
        sgtitle(['Monoc ROI Response to Non-attentional Binoc Block in Sess ' num2str(sessNr{cDay,2})])

        savefig(gcf, 'dffAcrossBinocHitTrials_blockSwitched')
        % saveas(gcf, 'dffAcrossBinocHitTrials.png')
    end

    if exist("avgSessionM", "var") % monoc ROI response to binoc block (non attn)
        figure;
        ...
        sgtitle(['Binoc ROI Response to Non-Attentional Monoc Block in Sess ' num2str(sessNr{cDay,1})])
        savefig(gcf, 'dffAcrossMonocHitTrials_blockSwitched')
    ...
    if exist("bTrNr", "var")
        ...
        savefig(gcf, 'peak_vs_BinocHitTrialNumber_blockSwitched')
        % saveas(gcf, 'peak_vs_BinocHitTrialNumber_blockSwithced.png')
    end
    ...
    if exist("mTrNr", "var")
        ...
        set(findobj(gcf, 'type', 'axes'), 'FontSize', 12)
        savefig(gcf, 'peak_vs_MonocHitTrialNumber_blockSwitched')
