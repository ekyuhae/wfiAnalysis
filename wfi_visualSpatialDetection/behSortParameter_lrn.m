function [positionsAndTimes, paramSorted] = behSortParameter_lrn(beh_all, block, parameters, bFrameTimes, vFrameTimes, stimOn, type, stimLocation)
...
bcont = parameters.targetContrast(:, find(parameters.targetAzimuth == stimLocation,1, 'first'))'; %CHANGED
bcont = sort(bcont);
loc = unique(parameters.targetAzimuth); % get location
param(1,:) = repelem(loc,1,length(bcont));
param(2,:) = [bcont]; %CHANGED
