...
% change per animal
Animal = 'M230831_1';
expID_ret = '06-Sep-2023'; %'04-Apr-2023_1'; % for retinotopy registration and identifying the visual areas 

% change per day
expID = '16-Sep-2023'; % this is WFI experiment ID format

% change per session
sessNr = 4; % enter the imaging session number(X) from Frames_2_xxx_xxx_uint16_000X 
expInfoNr = '4'; % enter the corresponding expInfo session number (does not always match with WFI sessNr) 
stimLocation = 45; %CHANGE
...
if stimulusTriggered
    ...
    if strcmp(dayType, 'passive') % 2 contrast levels (e.g., 0, 0.85, 0.85, 0.85), 1 location
        ...
        % hits
        idx = cellfun(@(x) find(x~=0,1,'first'), paramSorted_lrn.H, 'UniformOutput', false);
        ...
        % misses
        idx = cellfun(@(x) find(x~=0,1,'first'), paramSorted_lrn.M, 'UniformOutput', false);
        idx = find(~cellfun('isempty',idx)==1,1,'first');
        ...
        % false alarms
        idx = cellfun(@(x) find(x~=0,1,'first'), paramSorted_lrn.FA, 'UniformOutput', false);
        idx = find(~cellfun('isempty',idx)==1,1,'first');
        ...
        % correct rejects
        idx = cellfun(@(x) find(x~=0,1,'first'), paramSorted_lrn.CR, 'UniformOutput', false);
        idx = find(~cellfun('isempty',idx)==1,1,'first');
