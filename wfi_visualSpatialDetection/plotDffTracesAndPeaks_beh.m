function [temp] = plotDffTracesAndPeaks_beh(type, Animal, expID, hva, flag)
% Inputs:
...
try %load previously compiled data or add on new dates if necessary
    if isempty(hva)
        ...
        load([prev_path '\' expID{end} '\compiled analyzed data_' hva '.mat'])
    end 
    comp = analyzed_comp;
    ...
    [comp, nrSessions] = compileAnalyzedData_NA (fPath, Animal, expID, gratings, hva, flag); % changed from expID to explist EK %removed third output NA 12/4/23
end
...
for exp = 1:size(comp,1)
    for s = 1: size(comp,2)
        if ~isempty(comp(exp,s).whole)
            ...
            if numel(find(comp(exp,s).whole.contrasts == 0)) > 1
                ...
                if comp(exp,s).whole.paramSorted{1,1}(1) < 45 % binoc only
                    bcontrasts{exp,s} = comp(exp,s).whole.contrasts;
                    mcontrasts{exp,s} = [];
                else
                    mcontrasts{exp,s} = comp(exp,s).whole.contrasts;
                    bcontrasts{exp,s} = [];
                end
            end
        end
...
for exp = 1: length(explist)%newExpNr%:length(avg_sessions) %NA 12/4/23
    if any((any(cellfun(@(x) ~isempty(x), peak_value))))
        ...
        if isempty(hva)
             if ~isfolder('V1_compiled_sessions')
                mkdir('V1_compiled_sessions')
             end
             cd([directory filesep explist{exp} filesep 'V1_compiled_sessions'])
            savefig(f, ['meandff_over_contrasts_sessions_' type '.fig'])
            saveas(f, ['meandff_over_contrasts_sessions_' type '.png'])
        else
            if ~isfolder(hva)
                mkdir([hva '_compiled_sessions'])
            end
            cd([directory filesep explist{exp} filesep hva '_compiled_sessions'])
...
for exp = 1: length(avg_sessions)
    if ~isempty(temp{exp})
        ...
        % yLim = [min([min(min(traceb-stdb)); min(min(tracem-stdm))]) max([max(max(traceb+stdb)); max(max(tracem+stdm))])];
       yLim = [-2.5 8.5];
...
cd(directory)
if isempty(hva)
     if ~isfolder(['V1_compiled_days'])
        mkdir(['V1_compiled_days'])
    end
    ...
    if ~isfolder([hva '_compiled_days'])
        mkdir([hva '_compiled_days'])
    end
