function [positionsAndTimes, paramSorted] = behSortParameter_RT(beh_all, block, parameters, bFrameTimes, vFrameTimes, stimOn, type)
...
%         end
    case 'hit'
        ...
        for i = 1:length(idx)
            ...
            positionsAndTimes(i,2) = block.trial(1,idx(i)).inputThresholdCrossedTime;
...
%         end
    case 'Late'
        ...
        for i = 1:length(idx)
            positionsAndTimes(i,1) = block.trial(1,idx(i)).condition.targetAzimuth;
            positionsAndTimes(i,2) = block.trial(1,idx(i)).gratingStartedTime + beh_all.late_misses{idx(i)}(1);
            positionsAndTimes(i,3) = block.trial(1,idx(i)).condition.trialContrast; 
        end
end 
