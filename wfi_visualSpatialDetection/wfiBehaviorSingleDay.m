...
Animal = 'M230831_1';
expID = '09-Oct-2023'; % this is WFI experiment ID format
hva = 'RL_AM'; %higher visual areas being analyzed (i.e. 'RL_AM' or 'LM_PM' or '' for V1)
expID_ret = '06-Sep-2023'; %'28-Jul-2023'; %'04-Apr-2023_1'; % for retinotopy registration and identifying the visual areas 
% expID_ret = '16-Mar-2023';
sessNr_multi = [1 2 3 4 5 6]; % enter the imaging session number(X) from Frames_2_xxx_xxx_uint16_000X 
expInfoNr_multi = {'2' '3' '4' '5' '6' '7'}; % enter the corresponding expInfo session number (does not always match with WFI sessNr) 
...
for sessNr = sessNr_multi % EK CHANGE
    expInfoNr = expInfoNr_multi{find(sessNr == sessNr_multi)};
...
[~,~,out, beh_all, activeOut] = attention_effects_EK(Animal, date, {expInfoNr}); % 8/29 EK changed to attention code from regular analysis code
close all
