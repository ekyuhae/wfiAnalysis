...
[comp, nrSess] = compileAnalyzedData(fPath, Animal{1}, expID, gratings);
...
temp_gr = cell(1,nrLocs); % exp x location
...
for exp = 1: size(avg_sessions,1)
    for j = 1: size(avg_sessions, 2) % locations
        for i = 1:nrContrasts
            [peak_value(i,j), peak_index] = getPeakPostStim(avg_sessions{exp,j}(i,:), 15, 1, 2, []); %unclear what last input should be
    ...
    if gratings
        direc = [fPath filesep Animal{1} filesep 'gratings'];
    else
        direc = [fPath filesep Animal{1} filesep 'barmapping'];
    end
    cd([direc filesep expID{exp}])
...
for j = 1: size(avg_days, 2) % locations
    for i = 1:nrContrasts
        [peak_value_days(i,j), peak_index] = getPeakPostStim(avg_days{j}(i,:), 15, 1, 2,[]); %unclear what last input should be
