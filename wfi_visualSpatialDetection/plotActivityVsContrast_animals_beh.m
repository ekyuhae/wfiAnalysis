%% load individual session datasets that were analyzed and compile them 
% 
% EK 23 
...
Animal = {'M230831_1' 'M230717_1'};
% expID = {'03-Feb-2023', '08-Feb-2023_2','18-Feb-2023', '09-Mar-2023'};
expID = {'26-Oct-2023', '24-Oct-2023'};
...
for m = 1: length(Animal)
    ...
    if isempty(hva)
        cd([dir filesep 'V1' RT '_compiled_days'])
        data(m) = load(['compiled ROI traces_days_' type '.mat']);
    else
        cd([dir filesep hva RT '_compiled_days'])
...
cd(dir)
if isempty(hva)
    savefig(f, ['meandff_over_contrasts_animals_' type RT '.fig'])
    saveas(f, ['meandff_over_contrasts_animals_' type RT '.png'])
    save(['compiled ROI traces_animals_' type RT '.mat'], 'temp', 'Animal', 'data', 'avg_animal', 'std_animal')
else
    savefig(f, ['meandff_over_contrasts_animals_' type '_' hva RT '.fig'])
    saveas(f, ['meandff_over_contrasts_animals_' type '_' hva RT '.png'])
    save(['compiled ROI traces_animals_' type '_' hva RT '.mat'], 'temp', 'Animal', 'data', 'avg_animal', 'std_animal')
end
%% 
figure; 
subplot 121
...
set(ax,'ylim',yLim);
if isempty(hva)
    savefig(gcf, ['meandffTrace_animals_' type RT '.fig'])
    saveas(gcf, ['meandffTrace_animals_' type RT '.png'])
else
    savefig(gcf, ['meandffTrace_animals_' type RT '_' hva '.fig'])
    saveas(gcf, ['meandffTrace_animals_' type RT '_' hva '.png'])
end
clear data temp
end
%%
