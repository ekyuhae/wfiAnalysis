%% plot mean peak dF/F vs contrast across animals with licking related activity removed for hit & miss trials
% EK 23 
...
for m = 1: length(Animal)
    dir = [fPath Animal{m} filesep 'behavior'];
    if isempty(hva)
        stimTrigFA(m) = load([dir filesep 'V1_compiled_days' filesep 'compiled ROI traces_days_FA.mat']);
        lickTrigFA(m) = load([dir filesep 'V1_RT_compiled_days' filesep 'compiled ROI traces_days_FA.mat']);
    else
        stimTrigFA(m) = load([dir filesep hva '_compiled_days' filesep 'compiled ROI traces_days_FA_' hva '.mat']);
        lickTrigFA(m) = load([dir filesep hva RT '_compiled_days' filesep 'compiled ROI traces_days_FA_' hva '.mat']);
    end
end

nrContrasts = length(stimTrigFA(1).idxb);
for m = 1:length(Animal)
    magsFA(m).binoc = getPeakEvokedAmp(lickTrigFA(m).avg_exp{1}, stimTrigFA(m).avg_exp{1}, 15, 2);
    magsFA(m).monoc = getPeakEvokedAmp(lickTrigFA(m).avg_exp{2}, stimTrigFA(m).avg_exp{2}, 15, 2);
end

mags_all_b_fa = [magsFA(1).binoc magsFA(2).binoc];
mags_all_m_fa = [magsFA(1).monoc magsFA(2).monoc];

peak_value_fa(:,1) = nanmean(mags_all_b_fa,2); 
peak_value_fa(:,2) = nanmean(mags_all_m_fa,2); 
std_peak_fa(:,1) = nanstd(mags_all_b_fa,0,2);
std_peak_fa(:,2) = nanstd(mags_all_m_fa,0,2);
%% misses
...
f = figure;
subplot(1,2,1)
errorbar(stimTrig_miss(1).idxb*100, [NaN; peak_value_miss(2:end,1)*100], [NaN; std_peak_miss(2:end,1)*100], 'o-', 'color', colorM, 'LineWidth', 1.5, 'CapSize',0,'MarkerFaceColor',colorM);
hold on; 
errorbar(stimTrig(1).idxb*100, [NaN; peak_value(2:end,1)*100], [NaN; std_peak(2:end,1)*100], 'o-', 'color', color, 'LineWidth', 1.5, 'CapSize',0,'MarkerFaceColor',color); 
errorbar(stimTrigFA(1).idxb(1)*100, peak_value_fa(1,1)*100, std_peak_fa(1,1)*100, 'o-', 'color', color-0.3, 'LineWidth', 1.5, 'CapSize',0, 'MarkerFaceColor', color-0.3);
...
legend('Miss', 'Hit', 'FA', 'Location', 'northwest'); legend boxoff
axis square; axis padded; box off

subplot(1,2,2)
errorbar(stimTrig_miss(1).idxm*100, [NaN; peak_value_miss(2:end,2)*100], [NaN; std_peak_miss(2:end,2)*100], 'o-', 'color', colorM, 'LineWidth', 1.5, 'CapSize',0,'MarkerFaceColor',colorM);
hold on;
errorbar(stimTrig(1).idxm*100,  [NaN; peak_value(2:end,2)*100], [NaN; std_peak(2:end,2)*100], 'o-', 'color', color, 'LineWidth', 1.5, 'CapSize',0,'MarkerFaceColor',color); 
errorbar(stimTrigFA(1).idxb(1)*100, peak_value_fa(1,2)*100, std_peak_fa(1,2)*100, 'o-', 'color', color-0.3, 'LineWidth', 1.5, 'CapSize',0, 'MarkerFaceColor', color-0.3);
...
mags_all.fa.binoc = mags_all_b_fa; 
mags_all.fa.monoc = mags_all_m_fa;
...
for i = 2:nrContrasts
    c = [k(i), k(i), k(i)];
    errorbar(x(1:2), [peak_value_miss(i,1)*100 peak_value(i,1)*100], [std_peak_miss(i,1)*100 std_peak(i,1)*100], 'o-', 'CapSize',0,'color',c,'MarkerFaceColor',c); 
    hold on
end
    errorbar(3,  peak_value_fa(1,1)*100, std_peak_fa(1,1)*100, 'o-', 'CapSize',0,'color',color-0.3,'MarkerFaceColor',color-0.3); 

set(gca, 'XTick', x, 'XTickLabel', x_labels);
axis padded; axis square; box off
set(findobj(gcf, 'type', 'axes'), 'xlim', [0 4], 'ylim', [-1 3])
legend('1%', '2%', '5%', '10%', '18%', 'Location','northwest'); legend boxoff
...
for i = 2:nrContrasts
    c = [k(i), k(i), k(i)];
    errorbar(x(1:2), [peak_value_miss(i,2)*100 peak_value(i,2)*100], [std_peak_miss(i,2)*100 std_peak(i,2)*100], 'o-', 'CapSize',0,'color',c,'MarkerFaceColor',c); 
    hold on
end
    errorbar(3,  peak_value_fa(1,2)*100, std_peak_fa(1,2)*100, 'o-', 'CapSize',0,'color',color-0.3,'MarkerFaceColor',color-0.3); 

set(gca, 'XTick', x, 'XTickLabel', x_labels);
axis padded; axis square; box off
set(findobj(gcf, 'type', 'axes'), 'xlim', [0 4],  'ylim', [-1 3])
legend( '2%', '5%', '10%', '18%', '33%','Location','northwest'); legend boxoff
ylabel('Mean Peak Value (%)');
if isempty(hva)
    title('Monocular V1');
else
