...
expID = {'02-Mar-2023','02-Mar-2023_1'};
...
subplot(1,2,1)
if gratings 
errorbar(analyzed_comp(end,1).contrasts, peak_value(:,1), std_peak(:,1), 'o');
else 
    errorbar(cont, peak_value(:,5), std_peak(:,5), 'o');
end 
xlabel('Contrasts (%)')
ylabel('\Deltaf/f');
...
subplot(1,2,2)
if gratings 
errorbar(analyzed_comp(end,1).contrasts, peak_value(:,2), std_peak(:,2), 'o');
else 
...
[trace,std] = getAvgTraceAcrossStimLocs(avg_animal, cont, 30);
