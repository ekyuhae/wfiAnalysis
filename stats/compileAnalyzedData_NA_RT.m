function [analyzed_comp, nrSessions, startIdx] = compileAnalyzedData_NA_RT (fPath, Animal, expID, gratings, hva, flag)
...
if flag == 1
    if isempty(hva)
        afile = [prev_path filesep list(startIdx-1).name filesep 'compiled analyzed data_RT.mat']; %EK changed
    else
        afile = [prev_path filesep list(startIdx-1).name filesep 'compiled analyzed data_' hva '_RT.mat']; %EK changed
    end
else
    if isempty(hva)
        afile = [prev_path filesep list(startIdx).name filesep 'compiled analyzed data_RT.mat']; %EK changed
    else
        afile = [prev_path filesep list(startIdx).name filesep 'compiled analyzed data_' hva '_RT.mat']; %EK changed
    end
end
...
for exp = 1:length(expID)
    ...
    for i = 1:length(sessions)
        if regexp(sessions(i).name, '^\d+$')
            ...
            if isempty(hva)
                p = [dir_path filesep num2str(sessNr) filesep 'V1_RT'];
            else
                p = [dir_path filesep num2str(sessNr) filesep hva '_RT'];
            end
            if isfolder(p)
            cd(p)
            ...
            elseif gratings ==2 %behavior
                if isempty(hva)
                    cfile = ['analyzed_active_' num2str(sessNr) '_hit.mat'];
                    if exist(cfile)
                    ...
                    cfile = ['analyzed_active_-_' hva '_hit.mat'];
                     if exist(cfile)
                        % analyzed_comp(exp, sessNr).whole= load(cfile);
                        analyzed_comp(exp, sessNr).hit= load(['analyzed_active_-_' hva '_hit.mat']);
                        ...
                        analyzed_comp(exp, sessNr).LL= load(['analyzed_active_-_' hva '_Late.mat']);
                        analyzed_comp(exp, sessNr).FA= load(['analyzed_active_-_' hva '_FA.mat']);
                    end
                end
            end
            end 
...
if isempty(hva)
    save ([dir_path filesep 'compiled analyzed data_V1_RT.mat'], 'analyzed_comp');
else 
    save ([dir_path filesep 'compiled analyzed data_' hva '_RT.mat'], 'analyzed_comp');
end
