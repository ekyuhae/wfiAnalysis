function [analyzed_comp, nrSessions, startIdx] = compileAnalyzedData_NA (fPath, Animal, expID, gratings, hva, flag)
...
if flag == 1
    if isempty(hva)
        afile = [prev_path filesep list(startIdx-1).name filesep 'compiled analyzed data.mat']; %EK changed
    else
        afile = [prev_path filesep list(startIdx-1).name filesep 'compiled analyzed data_' hva '.mat']; %EK changed
    end
else
    if isempty(hva)
        afile = [prev_path filesep list(startIdx).name filesep 'compiled analyzed data.mat']; %EK changed
    else
        afile = [prev_path filesep list(startIdx).name filesep 'compiled analyzed data_' hva '.mat']; %EK changed
    end
end
