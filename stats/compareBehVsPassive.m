...
for j = 1: size(avg_animal_p, 2) % locations
    for i = 1:nrContrasts
        [peak_value_p(i,j), peak_index] = getPeakPostStim(avg_animal_p{j}(i,:), 15, 1, 2);
...
for j = 1: size(avg_animal_a, 2) % locations
    for i = 1:nrContrasts_a
        [peak_value_a(i,j), peak_index] = getPeakPostStim(avg_animal_a{j}(i,:), 15, 1, 2);
...
figure;
 bar ([0 2 5 10 33], percent_increase)
axis padded; axis square; box off
set(findobj(gcf, 'type', 'axes'), 'xlim', [-5 40])
xlabel('Contrast (%)', 'FontSize', 12); ylabel('Increase Percentage (%)', 'FontSize', 12)
 %%
f = figure;
subplot(1,2,1)
