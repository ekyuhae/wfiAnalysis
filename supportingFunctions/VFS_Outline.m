...
subplot(1,3,2); imagesc(azimuth, [-60 140]);colormap jet;colorbar; daspect([1 1 1]); hold on;
contour(nanmean(imout,3),'k'); hold off; 
title([Animal ' - Azimuth+Contour']); axis image
% El with area overlay
subplot(1,3,3); imagesc(elevation, [-40 50]);colormap jet;colorbar; daspect([1 1 1]); hold on;
contour(nanmean(imout,3),'k'); hold off; 
