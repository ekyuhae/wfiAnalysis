...
fr = fr/2;
t = 0:1/fr:(size(traces{1,1},2)-1)/fr; % convert frame into timescale 
t = t - t(fr*2+1); 
% t = t - t(floor(fr*0.5)+1);
k = linspace(0.8,0.2, length(contrasts));
...
for i = 1: size(avg_locs,1)
 % not including the contrast that weren't used in the experiment days (excluding NaN rows)
...
idx =cellfun(@(x) ~isempty(x), legend_names);
legend(lineout(idx), legend_names(idx)); legend('boxoff');
