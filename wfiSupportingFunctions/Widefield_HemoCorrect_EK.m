function data = Widefield_HemoCorrect_EK(data,hemoData,baseline, baselinePix, smoothFact)
...
% getting dF/F for each data 
data = single(data); 
[A,B,C] = size(data);
dataAvg = mean(data(baselinePix(1),baselinePix(2),baseline),3);
...
hemoAvg = mean(hemoData(baselinePix(1),baselinePix(2),baseline),3);
