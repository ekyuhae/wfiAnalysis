function dataCorrected = Widefield_HemoCorrect(data,hemoData,baseline,smoothFact)
...
% a = mean(data,1); b = mean(hemoData,1);
% figure; plot (a); hold on ; plot(b)
% 
% ylabel('df/f'); xlabel('frame number');
%% smooth hemo data
...
dataCorrected = data - hemoData; %subtract scaled hemoChannel from data
dataCorrected = reshape(dataCorrected,A,B,C);
dataAvg = mean(dataCorrected(:,:,baseline),3);
dataCorrected = bsxfun(@minus, dataCorrected, dataAvg); % correct baseline offset

% c = mean(reshape(dataCorrected,[],C),1);
% plot(c); legend ('Average Blue', 'Average Violet', 'Corrected')
